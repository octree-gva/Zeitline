...
var Timeline = function () {
  ...
  _createClass(Timeline, [{
    ...
    value: function init() {
      ...
      this.timeline = this.svg.append('g').attr('class', 'timeline').attr('transform', 'translate(' + this.margin.left + ',' + this.margin.top + ')');
      this.axis = this.timeline.append('g').attr('class', 'axis axis--x').attr('transform', 'translate(0,' + this.positionY + ')');
    }
    ...
    value: function renderAxis() {
      ...
      var range = [0].concat(_toConsumableArray(pivots), [this.width]);
      ...
      this.x = d3.scaleTime().domain([d3.extent(this.dateRange)[0]].concat(_toConsumableArray(domain), [d3.extent(this.dateRange)[1]])).range(range); // all pivots

      // Create axis with the given tick interval
      this.xAxis = d3.axisBottom(this.x).ticks(d3['time' + this.ticksIntervals]) // timeDay, timeWeek, timeMonth, timeYear
      .tickPadding(-70).tickSize(20);
      // .tickFormat(d3.time.format('%Y'),)

      // Draw axis
      this.axis.transition().duration(500).call(this.xAxis);

      // Remove old intervals separation if needed
      this.timeline.selectAll('.reference-line').remove();

      // Draw intervals separation
      range.forEach(function (pivot) {
        _this.timeline.append('line').attr('class', 'linear reference-line').attr('x1', +pivot - 0.5).attr('x2', +pivot - 0.5).attr('y1', _this.positionY - 30).attr('y2', _this.positionY + 30);
      });
    }
    ...
    value: function getPivots(dateRange, intervals) {
      ...
      // Compute each pivot
      return intervals.reduce(function (res, interval) {
        var xInterpolate = xMain(interval[0]);
        return {
          pivots: res.pivots.concat([xInterpolate + res.eaten, xMain(interval[0]) + interval[2] + res.eaten]),
    ...
    value: function renderData(data) {
      ...
      this.timeline.selectAll('circle').data(data).enter().append('circle').attr('class', 'dot').attr('cx', function (d) {
        return _this2.x(d.date);
      }).attr('cy', this.positionY).attr('r', 0).transition().attr('r', 5);

      var that = this;
      this.timeline.selectAll('circle').on('click', function (circle) {
        d3.select(this).transition().ease(d3.easeBounceOut).duration(500).attr('r', 15).transition().duration(500).call(function () {
          if (that.onClick) {
            that.onClick.apply(circle);
          }
          // d3.event.stopPropagation();
        }).delay(500).attr('r', 5); // reset size
      });
    }
